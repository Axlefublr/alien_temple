...
impl DiscoveryRepo {
    ...
    fn save(self) -> Result<(), &'static str> {
        fs::write(DISCOVERY_FILE, self.contents)
            .map_err(|_| "couldn't write to discovery file")
    }
}

fn parse() -> Result<String, &'static str> {
    let contents = fs::read_to_string(DISCOVERY_FILE)
        .map_err(|_| "couldn't read discovery file")?;
    Ok(contents)
}
