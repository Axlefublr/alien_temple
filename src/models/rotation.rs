...
impl RotateRepo {
    ...
    pub fn has(&self, artist: &str) -> bool {
        self.contents
            .lines()
            .any(|line| line.split(" — ").nth(1).unwrap_or_default().to_ascii_lowercase() == artist.to_ascii_lowercase())
    }
    ...
    pub fn remove(mut self, artist: &str) -> Result<(), &'static str> {
        self.contents = self
            ...
            .lines()
            .filter(|line| {
                line.split(" — ").nth(1).expect("rotate file doesn't use —")
                    != artist
            })
            .map(|line| line.to_owned())
            .collect::<Vec<_>>()
    ...
    fn save(self) -> Result<(), &'static str> {
        fs::write(ROTATE_FILE, self.contents)
            .map_err(|_| "couldn't write to rotate file")
    }
}

fn parse() -> Result<String, &'static str> {
    let contents = fs::read_to_string(ROTATE_FILE)
        .map_err(|_| "couldn't read rotate file")?;
    Ok(contents)
}
