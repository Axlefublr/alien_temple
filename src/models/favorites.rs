...
impl FavoriteRepo {
    ...
    fn save(self) -> Result<(), &'static str> {
        fs::write(FAVORITE_FILE, self.contents)
            .map_err(|_| "couldn't write to favorites file")
    }
}

fn parse() -> Result<String, &'static str> {
    let contents = fs::read_to_string(FAVORITE_FILE)
        .map_err(|_| "couldn't read favorites file")?;
    Ok(contents)
}
